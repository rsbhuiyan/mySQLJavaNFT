...
public class userDAO 
{
    ...
    public boolean buyNFT(int NFTID, String prevOwner, String owner) throws SQLException{
    	String sql = "UPDATE User SET cash_bal = cash_bal + (SELECT price FROM NFT WHERE NFTID = ?) WHERE email= ?";
        connect_func();
        
        preparedStatement = (PreparedStatement) connect.prepareStatement(sql);
        preparedStatement.setInt(1, NFTID);
        preparedStatement.setString(2, prevOwner);

        boolean rowUpdated = preparedStatement.executeUpdate() > 0;
        preparedStatement.close();
        buyNFT2(NFTID, prevOwner, owner);
        
        System.out.println("Buy successful! " + owner + " is now the new owner");
        return rowUpdated;     
			
	}
    

    public List<user> searchUser(String email1) throws SQLException {   

   	 List<user> searchUser = new ArrayList<user>(); 
   	 String sql = "SELECT email, cash_bal FROM User WHERE email LIKE ?";
     ...
     //mySQL username and password
  	connect_func("", "");   
        ...
        while (resultSet.next()) {
            ...
            user users = new user(email, cash_bal);
    ...
    public boolean buyNFT2(int NFTID, String prevOwner, String owner) throws SQLException{
    	String sql = "UPDATE User SET cash_bal = cash_bal - (SELECT price FROM NFT WHERE NFTID = ?) WHERE email= ?";
        connect_func();
